...
class CPPN:
	def __init__(self, n_inputs,n_outputs,t_config=None):
		...
		if (t_config is not None):
			...
			self.config.genome_config.conn_add_prob = float(t_config['ea']['mutation_prob'])
			self.config.genome_config.conn_delete_prob = float(t_config['ea']['mutation_prob'])
			self.config.genome_config.node_add_prob = float(t_config['ea']['mutation_prob'])
			self.config.genome_config.node_delete_prob = float(t_config['ea']['mutation_prob'])
	...
	def crossover(self,other):
		self.genome.configure_crossover(self.genom,other.genome,self.config.genome_config)
