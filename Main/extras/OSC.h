 ...
 *   buffer and re-use it for the returned values.
 ...
 *   On call this should be an empty buffer, and on return it will contain
 *   the values that were int the OSC message. The values are stored in a union,
 *   so returnedFormat must be queried to determine which member contains valid data.
 * @param returnedValues
 *   On call this should be the capacity of returnedValues.
 * @result
 ...
 * 
 */
int oscParse(char* buffer, int numValidCharsInBuffer, char** returnedAddress, char** returnedTypeTag, oscValue_t* returnedValues, int returnedValuesLen);
