...
/*-------------------------------------------------------------------------------------*/
list_entry_t* list_entry_with_data(List* self, list_data_t data, unsigned* returned_index)
{
  list_entry_t* this_entry = self->first_entry;
  unsigned index = 0;
...
void list_clean_entry(List* self, list_entry_t* entry)
{
  if(entry->deallocate_data != NULL)
    entry->deallocate_data(entry->data);
}
...
/*-------------------------------------------------------------------------------------*/
BOOL          list_contains_data      (List* self, list_data_t data, unsigned* returned_index)
{
  list_entry_t* e = list_entry_with_data(self, data, returned_index);
...
{
  list_entry_t* e = list_entry_with_data(self, data, NULL);
  list_remove_entry(self, e, should_deallocate);  
}
...
{
  list_entry_t* this_entry = list_entry_with_data(self, old_data, NULL);
  if(this_entry != NULL)
    {
      if(should_deallocate)
        list_clean_entry(self, this_entry);
...
/*-------------------------------------------------------------------------------------*/
void list_default_data_deallocator(list_data_t data)
{
  if(data != NULL)
    free(data);
}
