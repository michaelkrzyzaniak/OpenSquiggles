...
/*auNew---------------------------------------------------*/
Audio* auAlloc(int sizeofstarself, auAudioCallback_t callback, BOOL isOutput, unsigned numChannels, double sampleRate, int bufferNumFrames, int numBuffers)
{
  ...
  if(self != NULL)
    {
      ...
      self->bufferNumFrames              = bufferNumFrames;
      ...
      self->dataFormat.mSampleRate       = self->sampleRate;
      ...
      if(error) 
        ...
        {
          self->buffers = calloc(self->numBuffers, sizeof(*self->buffers));
          unsigned bufferNumBytes = self->bufferNumFrames * numChannels * sizeof(auSample_t);
      ...
      if(error >= 0)
        {
          unsigned int sampleRate = self->sampleRate;
      ...
      if(error >= 0)
        {
          unsigned long int internalBufferNumFrames = self->bufferNumFrames * self->numBuffers; //the buffer ALSA uses internally
          ...
          else if(internalBufferNumFrames != self->numBuffers * self->bufferNumFrames)
              fprintf(stderr, "Audio.c: device does not support %lu internal buffer size, %lu will be used instead\n", self->numBuffers * self->bufferNumFrames, internalBufferNumFrames);
        }
      if(error >= 0)
        {
