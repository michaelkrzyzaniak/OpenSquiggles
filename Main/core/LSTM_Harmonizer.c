...
struct opaque_lstm_harmonizer_struct
{
  ...
  Matrix* middle_layer_weights;
  Matrix* middle_layer_biases;
  Matrix* middle_layer_out;
  ...
  if(self != NULL)
    {
      ...
      self->middle_layer_weights= matrix_new_from_numpy_file(lstm_harmonizer_filename(folder, "middle_layer_weights.npy", buffer));
      self->middle_layer_biases = matrix_new_from_numpy_file(lstm_harmonizer_filename(folder, "middle_layer_biases.npy" , buffer));
      /*
      ...
      */
      ...
      if((self->layer_1_weights == NULL) || (self->layer_1_biases == NULL) ||
         (self->layer_3_weights == NULL) || (self->layer_3_biases == NULL) ||
         (self->middle_layer_weights == NULL) || (self->middle_layer_biases == NULL))
...
*/
      ...
      self->middle_layer_out    = matrix_new(self->num_layer_3_inputs, 1);
/*
...
*/
      ...
      */
      if((self->audio_features  == NULL) || (self->input_vector == NULL) || (self->layer_1_out == NULL) ||
         (self->middle_layer_out  == NULL) || (self->layer_3_out == NULL))
         {fprintf(stderr, "error allocating new matrices\r\n");  return lstm_harmonizer_destroy(self);}
  ...
  if(self != NULL)
    {
      ...
      self->middle_layer_weights = matrix_destroy(self->middle_layer_weights);
      self->middle_layer_biases  = matrix_destroy(self->middle_layer_biases);
      /*
...
Matrix* lstm_harmonizer_forward(LSTM_Harmonizer* self, Matrix* inputs)
{
  ...
  //middle layer
  matrix_multiply(self->middle_layer_weights, self->layer_1_out, self->middle_layer_out);
  matrix_add(self->middle_layer_out, self->middle_layer_biases, NULL);
  matrix_apply_function(self->middle_layer_out, lstm_harmonizer_relu, self, NULL);
  ...
  */
  ...
  matrix_multiply(self->layer_3_weights, self->middle_layer_out, self->layer_3_out);
  pthread_mutex_unlock(&self->clear_mutex);
